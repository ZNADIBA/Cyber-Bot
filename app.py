...
def main():
    st.title("Izzy, Your Cybersecurity Guide")

    # Izzy introduces himself
    st.write("Hey there! I'm Izzy, your go-to guy for advanced cybersecurity! 🌐")
    st.write("Let's start with your name... yeah? What's your name?")

    # Get user name
    user_name = st.text_input("Enter your name")

    # Create a session state to keep track of the fact index
    if 'fact_index' not in st.session_state:
        st.session_state.fact_index = 0

    if user_name:
        st.write(f"Awesome, {user_name}! Nice to meet yah! I'm here to teach you some cybersecurity facts about AI.")
        st.write("Whenever you're ready, just tap the **Get a Fact** button below to start learning something cool!")

        # Button to get a fact
        if st.button("Get a Fact"):
            # Detailed AI and Cybersecurity facts
            facts = [
    ...
    "250. AI in Security Metrics Analysis: AI analyzes security metrics providing insights into the effectiveness of security measures and areas for improvement."
        ...
        "300. AI for Global Cybersecurity Threat Intelligence: AI gathers information on cyber threats from around the world. This helps organizations stay informed about new dangers that could affect their security."
                      ...
                      # Show an introductory message before displaying the first fact
            if st.session_state.fact_index == 0:
                st.write("Okay, let’s learn about how AI is used to protect cybersecurity! Ready? Here’s a fact:")
            
            # Show the current fact in order
            if st.session_state.fact_index < len(facts):
                st.write(facts[st.session_state.fact_index])
                st.session_state.fact_index += 1
            else:
                st.write("You've learned all the facts! 🎉 Start over for more learning!")
